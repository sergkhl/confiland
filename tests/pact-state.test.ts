...
import { DAILY_KEY, DEMO_KEY, LEGACY_DAILY_KEY, PATH_ID, newState, parseState, migrateLegacy, transitionPact, type PactState, type PactEvent } from '../lib/pact-state.ts';
...
test('exactly six authored actions remain selectable with controllable criteria', () => {
...
test('Too much does not change choice; trace freezes all choice fields', () => {
...
test('explicit skip differs from an unanswered prediction or observation', () => {
...
test('seal uses actual date, never elapsed time, and rejects duplicate or stale events', () => {
...
test('round trips reject invalid dates, progress, identity and phase records', () => {
...
test('legacy signed actions and outcomes survive without new signature or social reinterpretation', () => {
...
test('failed compatibility writes preserve v1 and do not expose a committed v2 record', () => {
...
test('storage commits reread current state and save before exposing changes', () => {
...
test('daily and tab demo stores and replay remain independent', () => {
...
test('overdue pacts stay open until an explicit review; a closed day permits tomorrow', () => {
