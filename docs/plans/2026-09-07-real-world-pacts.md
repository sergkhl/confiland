...
Status: In progress; implementation and available browser checks pass; remaining acceptance needs additional test capabilities.
Order: 1 of 1.
...
NEXT: Resume remaining units 2/4 scenarios with true touch, motion inspection, reduced-motion configuration, and isolated browser storage/date fixtures.
Exit: Keep this plan open for the concrete unverified scenarios in Open findings; testing requires no permission question.
...
- **TOUCH-AND-MOTION (units 2 and 4)** — The available browser controls expose mouse segment drags, keyboard/button activation, screenshots, and viewport resizing, but no true touch trajectories, continuous motion inspection, or reduced-motion preference switching. Observe a continuous comfortable trace against the 8–12 second target, active signing/lift/answer motion, reduced-motion feedback, and touch behavior when these capabilities are available. Narrow screenshots and elapsed tool time do not establish those results. Mute UI behavior passed; audible output and vibration hardware remain unverified.
- **BROWSER-RECOVERY (unit 4)** — Deterministic fixtures cover storage failures, stale events, cancellation, rollover, and overdue states; browser checks cover partial-trace/endpoint reload and daily/demo isolation. UI evidence is still needed for storage failure/retry, repeated/overdue daily fixtures, and cancellation/hiding while an endpoint pointer is held. The current controls do not expose storage/date fault setup or a separately held pointer. These are test-capability limits, not owner-permission blockers.
