<!-- Hygiene: every active plan appears here, ordered by value per unit of effort. Each entry <=5 lines; blocked work keeps its merit position. Update with the plan header and TODO after each unit. Delete a plan only after its exit test, in a separate commit; git history is the archive. -->
