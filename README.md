...
The tracing surface also accepts keyboard activation for the next stroke, but never uses that activation to seal. The separate seal button requires a new explicit action after completion. The guardian's answering animation runs only after a successful new seal, not when reopening a saved pact.
...
`npm test` passes all 34 focused tests; `npx tsc --noEmit`, targeted lint for the changed implementation, and `npm run build` pass. State/storage, review, tool-contract, and path tests cover choice freezing, explicit answers, local-date limits, overdue closure, exact legacy preservation, failed writes, stale IDs, mode isolation, ordered turns and retraces, shortcuts, reversal, off-path movement, early release, completed-endpoint cancellation/reload, and coordinate normalization. Regression tests traverse every corner with rounded browser pixels at three surface widths and recover an older trace saved just short of a corner. Persisted fixture journeys exercise both practices, all three outcomes, and skipped/chosen prediction branches.

Observed local in-app browser rehearsals cover both practices and all three outcomes, including true/inconclusive/skipped predictions and clearing incompatible answers. Mouse drags traversed the ordered word with pauses and re-grabs, reversal preserved ink, and resizing/reloading retained progress. Keyboard and button alternatives each completed all 53 strokes; an extra keyboard activation and endpoint reload stayed unsigned until explicit sealing. Reopening a saved seal did not replay its mark animation. Demo outcomes remained separate from an unsigned daily state. Screenshots at 1280 × 900 and 390 × 844 checked composition, focus, brush clearance, atlas crops, and the answering mark. Mute toggling and its retained UI state were checked. All reported outcomes were demo test data, not real-world attempts or confidence-effectiveness evidence.

True touch input, active-gesture cancellation/hidden-page integration, reduced-motion preference switching, browser storage/date fault fixtures, and continuous animation/timing inspection remain unverified with the available browser controls. Mouse segment drags and narrow screenshots do not establish touch ergonomics or the comfortable 8–12 second tracing target. Audible output, physical finger occlusion, assistive technology behavior, and vibration hardware have not been verified. The owning [plan](docs/plans/2026-09-07-real-world-pacts.md) retains the evidence and concrete remaining scenarios; no additional permission is needed to run acceptance tests.
