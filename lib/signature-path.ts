...
export const PATH_D =
  `M ${SEGMENTS[0].a.x} ${SEGMENTS[0].a.y} ` +
  SEGMENTS.map((s) => `L ${s.b.x} ${s.b.y}`).join(' ');
export function distanceAt(progress: number): number {
  const p = Math.max(0, Math.min(1, progress)),
    index = Math.min(4, Math.floor(p * 5)),
    group = groups[index];
  return group.start + (group.end - group.start) * (p * 5 - index);
}
export function progressAt(length: number): number {
  const s = Math.max(0, Math.min(PATH_LENGTH, length)),
    index = groups.findIndex((g) => s <= g.end),
    group = groups[index];
  return s === PATH_LENGTH
    ? 1
    : (index + (s - group.start) / (group.end - group.start)) / 5;
}
function segmentAt(length: number): Segment {
  return (
    SEGMENTS.find((s) => length < s.end - 0.00001) ??
    SEGMENTS[SEGMENTS.length - 1]
  );
}
export function pointAt(progress: number): Point {
  const s = distanceAt(progress),
    segment = segmentAt(s);
  return mix(segment.a, segment.b, (s - segment.start) / segment.length);
}
export function nextStroke(progress: number): number {
  return progressAt(
    (
      SEGMENTS.find((s) => s.end > distanceAt(progress) + 0.001) ??
      SEGMENTS[SEGMENTS.length - 1]
    ).end,
  );
}
export function nextGuide(progress: number): string {
  const point = pointAt(progress),
    end = pointAt(nextStroke(progress));
  return `M ${point.x} ${point.y} L ${end.x} ${end.y}`;
}
export function normalizePoint(
  ...
  return {
    x: ((x - bounds.left) * VIEWBOX.width) / bounds.width,
    y: ((y - bounds.top) * VIEWBOX.height) / bounds.height,
  };
}
...
export function advanceTrace(progress: number, from: Point, to: Point): number {
  if (![progress, from.x, from.y, to.x, to.y].every(Number.isFinite))
    return progress;
  ...
  for (let i = 1; i <= steps; i++) {
    const point = mix(from, to, i / steps),
      segment = segmentAt(accepted);
    const anchor = mix(
      segment.a,
      segment.b,
      (accepted - segment.start) / segment.length,
    );
    if (distance(previous, anchor) <= CORRIDOR) {
      ...
      const onPath = mix(segment.a, segment.b, projection),
        candidate = segment.start + projection * segment.length;
      if (
        candidate > accepted &&
        candidate - accepted <= stride * 1.5 + 0.25 &&
        distance(point, onPath) <= CORRIDOR
      )
        accepted = candidate;
    }
    previous = point;
...
export class TraceSession {
  ...
  begin(
    ...
    if (this.pointer !== null || distance(point, pointAt(progress)) > CORRIDOR)
      return false;
  ...
  move(pointer: number, ritualId: string, point: Point): number {
    ...
    this.progress = advanceTrace(this.progress, this.previous, point);
    this.previous = point;
    return this.progress;
  }
  release(
    ...
    const seal =
      this.progress === 1 &&
      savedProgress === 1 &&
      distance(point, pointAt(1)) <= CORRIDOR;
    this.cancel();
    return seal;
  }
  cancel() {
